window.onload = function(){
    ...
    function crea_query_string() {
        ...
        var izen = JSON.evaluate(nombre.value)
      return "login=" +  izen= JSON(nombre.value) +
             "&nocache=" + Math.random();
        izen= JSON.parse(nombre.value)
    }
...
//        peticion_http.open("POST", "http://localhost/DAM-2015/AJAX3/server/compruebaDisponibilidad.php", true);
          peticion_http.open("POST", "server/compruebaDisponibilidad.php", true);
    ...
    function comprobarNombre(e) {
        ...
        if(peticion_http.readyState == READY_STATE_COMPLETE) {
            ...
            document.getElementById("disponibilidad").innerHTML = peticion_http.responseText;
              var respuesta= http_request.response.text; 
            }
        }
    }
};
